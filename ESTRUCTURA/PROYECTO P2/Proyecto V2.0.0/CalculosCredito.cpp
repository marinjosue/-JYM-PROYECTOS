...
void CalculosCredito::calcular_saldos_restantes(double monto, double valor_cuotas, ListaDoble<double>* saldos, Nodo<double>* cabeza_capitales){
    double saldo_actual = monto;

    while(cabeza_capitales != nullptr){
        double pago_capital = cabeza_capitales->get_valor();
        double saldo_restante = saldo_actual - pago_capital;

        saldos->insertar_cola(std::abs(saldo_restante));
        // Actualizar el saldo_actual para la siguiente iteración
        saldo_actual = saldo_restante;

        cabeza_capitales = cabeza_capitales->get_siguiente();
    }
}
