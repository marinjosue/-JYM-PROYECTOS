...
bool Persona::eliminarPersona(const std::string& cedula) {
    ...
    if (existeEnUsuarios(cedula)) {
        std::cout << "\nLa cedula esta presente en Usuarios.txt. No se puede eliminar la persona.\n";
        return false;
    }
    ...
    if (!archivoEntrada.is_open()) {
        std::cerr << "\nError al abrir el archivo Personas.txt para lectura.\n";
        return false;
    }
    ...
    if (!archivoSalida.is_open()) {
        std::cerr << "\nError al abrir el archivo Personas.txt para escritura.\n";
        return false;
    }
    ...
    std::cout << "\nPersona con cedula " << cedula << " eliminada correctamente de Personas.txt.\n";
    return true;
}
...
bool Persona::existeEnUsuarios(const std::string& cedula) {
    ...
    if (archivoUsuarios.is_open()) {
        while (archivoUsuarios >> cedulaArchivo) {
            ...
            // Leer y descartar los otros campos (nombre, apellido, id)
            archivoUsuarios.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        }
        archivoUsuarios.close();
    } else {
