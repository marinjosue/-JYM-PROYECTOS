...
void Movimientos::realizarMovimiento(const std::string& cedulaIngresada, const std::string& tipoMovimiento) {
    ...
    std::string rutaArchivo = "Usuarios/" + cedulaIngresada + ".txt";

    // Abre el archivo en modo de escritura (app: append)
    ...
    int deposito = 0, retiro = 0;
    Fecha fecha;

    // Solicita el monto según el tipo de movimiento
    if (tipoMovimiento == "Deposito") {
        ...
        std::cout << "Tipo de movimiento no válido. Ingrese 'Deposito' o 'Retiro'." << std::endl;
    ...
    std::ifstream lecturaArchivo(rutaArchivo);

    // Lee la última línea del archivo para obtener el saldo anterior
    if (lecturaArchivo.is_open()) {
        while (std::getline(lecturaArchivo, ultimaLinea)) {
            // Continúa leyendo hasta llegar al final del archivo
        }
        lecturaArchivo.close();

        // Procesa la última línea para obtener el saldo anterior
        std::stringstream ss(ultimaLinea);
        std::string fechaHora, depositos, retiros, saldo;
        ss >> fechaHora >> depositos >> retiros >> saldoAnterior;
    }

    // Calcula el saldo actual
    int saldoActual = saldoAnterior + deposito - retiro;

    // Escribe la nueva transacción en el archivo
    ...
    std::cout << "Movimiento registrado con éxito." << std::endl;
}
